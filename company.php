<!DOCTYPE html>
<html>
<head>
	<title>Company</title>
	<?php include('includes/head_links.php'); ?>
	<link rel="stylesheet" type="text/css" href="styles/company.css">
</head>
<body class="inside-page">
	<?php include('includes/main_nav.php'); ?>
	<div class="main-content">
		<h1>About Serial Metrics</h1>
		<p>
			Founded in 2010, Serial Metrics delivers data science solutions to 
			both enterprise companies and newly-minted start-ups.  
		</p>
		<p>
			Serial Metrics leads in delivering data science solutions that help 
			companies effectively mine their data to build algorithmic, 
			server-side applications that solve a variety of common business problems. 
			Companies leverage Serial Metrics APIs to offer their customers innovative 
			solutions such as dynamic pricing, highly-targeted personalized recommendations, 
			curated content, etc. With Serial Metrics, companies no longer need to address 
			their data science issues with traditional in-house means to derive solutions 
			from scratch; leveraging Serial Metrics&rsquo; suite of machine learning modules 
			solves the most complex data science problems facing today&rsquo;s companies.
		</p>
	</div>
	<img src="images/customers_page/sf_bay.jpg" class="full-width">
	<div class="main-content">
		<h1>Our Team</h1>
		<ul>
			<li><strong>Amir Behbehani</strong> - Founder / Algorithms</li>
			<li><strong>Emiko Higashi</strong> - Partnerships</li>
			<li><strong>Bjorn Cooley</strong> - Development</li>
			<li><strong>Igor Savic</strong> - Engineering</li>
			<li><strong>Jason Yu</strong> - Research</li>
			<li><strong>Ben Smucker</strong> - Sales</li>
			<li><strong>Imran Khalik</strong> - Legal</li>
			<li><strong>Katherin Yi </strong> - Data Science</li>
		</ul>
	</div>
	<?php include('includes/footer.php'); ?>
</body>
</html>